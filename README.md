...
3. Define your network:
 - Add processes: `network.defProc(...)`  Note: when several processes use the same component, `defProc` takes the process name as a second argument.
  The second argument can be an explicit name or a name of the form `root_XXX`, where `XXX` is replaced with a zero-padded
  number that increments each time the template is used.
...
3. Execute `npm install`
   Install requires the following `npm` packages: `parsefbp`, `fibers`, `mocha`, `chai` and `mocha-fibers` - you may have to do `npm` installs for some or all of these.
...
9. Install requires the following `npm` packages: `parsefbp`, `fibers`, `mocha`, `chai` and `mocha-fibers` - you may have to do `npm` installs for some or all of these.
